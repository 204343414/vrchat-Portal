...
//
// 【手持刚体塞进门后松开、出现在门后而非出口的修复（2026-08-13）】
//   症状：抓取刚体快速塞进A门后松开，刚体留在A平面后面，不传送到B出口。
//   根因：手持刚体是kinematic、靠MovePosition驱动，rb.velocity≈0；动态追踪深度扩展
//         （靠法线速度把追踪门槛从1.1米扩到最高8米，防高速隧穿）对它永远不生效。
//         快速一捅时刚体一帧内从门槛外跳到平面后侧，追踪器首次收录时previousOffset
//         已是"后面"的位置，前后侧比较永远没有前侧记录→穿越判定永不触发。
//   方案（第一版"在后侧即传"造成跨门映射握持被误传、拔河鬼畜，已推翻重做）：
//         管理器每帧缓存手持刚体上一帧位置；入口扫描中，新收录、当前在后侧、且
//         【上一帧在前侧→这一帧在后侧（线段真实穿过平面）】的手持刚体按"本帧完成穿越"处理：
//         前后帧插值出精确穿越点（含门框XY校验）立即传送，随后加入对面门追踪。
//         上一帧已在后侧的（跨门映射握持等合法常驻状态）只正常追踪，绝不传送。
//
// 【手持刚体折射握持重写（2026-08-13，B方向鬼畜/塞不进/松手消失的最终修复）】
//   症状：A打地板、B打斜面时，抓刚体塞进A→从B出正常；塞进B则鬼畜、像撞墙塞不进；
//         松手刚体整个消失（掉进出口侧墙体/虚空）。
//   根因：旧握持映射【无条件】把"枪口前方1米"的握持点镜像到出口侧——当玩家把枪拉回、
//         握持点回到门平面前侧时，镜像点落到出口平面【后方】（A是地板门=地板下方虚空，
//         B是斜面门=斜坡实体内部）：刚体被拖进几何→追踪器检测到穿越→传送拉出→映射再拖回，
//         每帧拔河=鬼畜+塞不进；松手时刚体正好在几何内部→掉进夹层/虚空=消失。
//   方案（采纳之前LLM的"抓取射线两段折射"思路，推广到握持定位）：
//         1) 传送枪 Update 未映射分支：握持射线穿过某扇门(落在门框内)→刚体直接放到折射位置
//            并建立映射——塞入过程平滑、A/B天然对称、刚体永不进墙；
//         2) 已映射分支加"折射门控"：仅当枪身已过门平面或握持射线仍穿过该门才继续应用映射，
//            否则清除映射、刚体回到玩家侧直接握持点——镜像点永远不会落在出口平面后方。
//         管理器新增 TryGetPortalCrossingForSegment（复用TryRayPortalIntersection，含形状判定、
//         双门取近）供枪侧调用。持物跟人穿门的既有状态机语义不变。
// ================================================================================
#if UNITY_EDITOR
