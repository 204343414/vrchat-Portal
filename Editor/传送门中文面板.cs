...
// 最后更新：2026-08-18（二十五轮：出口贴面0.06m——近垂直入射可见死区消失）
// ================================================================================
//
// 【项目结构，一共3个脚本】
...
//     二十五轮（2026-08-18，出口贴面0.06m：近垂直入射可见死区消失）：
//           用户实测（stretched billboard、1.0×1.0×1.0、近90°入射）报残留现象：
//           "出口中间断开/出口原点沿出口法线有段距离"——事件日志出射面距=0.20恒定。
//           破案：二十四轮沿光束前推0.2m，近垂直入射时法向分量≈0.2m=出口光束与门面
//           之间的可见死区（0.2m内无粒子=断开感）。
//           修正：前推量改为"法向分量恰好=offset+0.01"（最小安全值——必须越过出口检测面
//           z=offset，否则下帧线段会再穿检测面被规则1回抓=乒乓；这是防回穿的必要距离，
//           不能再小）。近垂直入射时出口贴门面0.06m（旧0.2m死区缩小3倍+）；
//           切线入射沿光束推距封顶2m后补沿出口侧法线推足。日志验证标准更新：
//           出射面距应≈±(offset+0.01)≈0.06。
//           诚实边界（用户问"触发器兜底"与"穿一段短距离"）：
//           a) 触发器路线死路：OnParticleTrigger 在 Udon 编译不过（CS0115 实测记录在案），
//              OnTriggerEnter 对粒子不触发（粒子不是刚体）；ParticlePhysicsExtensions
//              至今未开放（VRChat反馈板 tracked）——触发器级兜底在 Udon 不存在。
//           b) "速度100穿一小段短距离才被传"=帧采样固有边界：粒子必须在穿面后的首个
//              采样帧才可见，100m/s时该帧深度≈1.7m（1000时16.7m），渲染一帧后被传走。
//              不是漏（滞留=0），是采样延迟；除非引擎开放粒子回调，无解。
//           c) 用户已把 teleportTriggerOffset 调至0.1（实测前进视角不闪屏的最佳值；
//              倒退闪屏仍在，属渲染课题留待后轮）。
//     二十四轮（2026-08-18，出射连续性终重构：断层/散射根治）：
//           用户实测（速度1000、187粒/秒、斜射）报"断层/水中折射"：出射光束方向正确
//           但原点错开，角度越大越明显；速度10无感、100略、1000明显（速度相关）。
//           破案（事件日志出射面距自证=-33~-51米）：出射点离出口门面几十米——
//           两个叠加机制：
//           a) remainingTime 帧内积分：粒子过门后按剩余帧时间继续飞（速度×剩余时间），
//              1000m/s低帧率下一推几十米→门面上留无粒子缝隙=断层，速度越快缝隙越大；
//           b) 锚点与前推的横向分量：锚点在检测面(z=offset)上（斜射横偏offset×tanθ）+
//              前推沿出口法线landingPush（斜射把整条出口光束横移landingPush×cosθ）。
//           修复（TeleportParticleThroughPortal 重构）：
//           1) 锚点统一回移 offset 到真实门平面(z=0)——真实过门点；
//           2) 前推改为沿【出口光束方向】0.2m——出口光束与入口光束的映射像严格同一直线
//              （零横向断层）；切线飞行兜底沿出口侧法线补推到≥0.05m；
//           3) 删除 remainingTime 帧内积分——出射粒子恒紧贴门面0.05~0.2m，光束在门上
//              连续。代价：粒子传送帧损失(1-t)×dt运动（次帧配对从出口继续，不可见）。
//           日志预期（验证标准）：事件行"出射面距"应≈±0.05~0.2（符号随门forward朝向），
//           旧版的几十米负值应消失；同一光束所有粒子出射点收敛到门面附近。
//           附带释疑（用户疑"一会规则1一会规则2"）：规则号=哪条检测规则抓到粒子
//           （1穿越帧/2框内门后补抓/3未配对兜底/4反弹重建），与Simulation Space无关，
//           同一光束的粒子按进入方式被不同规则抓，属正常。
//           附带提醒：teleportTriggerOffset 是【玩家】触发偏移（与粒子无关，粒子检测面
//           偏移是particleTeleportPlaneOffset）；用户曾调0排查，需调回0.3（十五轮记录：
//           0会让玩家头卡"近门渲染退化区"）。
//     二十三论（2026-08-18，出射锚定终版：沿运动线反推过面点——散射归零）：
//           用户实测（速度1000、187粒/秒、斜射）报"出口光束像水中折射"：方向对但
//           初始位置在门框范围内随机散射；90°垂直入射不散射，81.7°/93.4°等特定角度散射。
//           破案（事件日志+漏检嫌疑行自证）：规则2捕获"漂进门框"的粒子时，二十二轮
//           用沿法线投影锚定=保留粒子【当前】x/y——粒子漂进框时在框内哪个位置是随机的
//           （高速斜射漂到49米深才进框，日志实测入深48.98），出口x/y随之在框内随机散射。
//           终版：hitPoint 改为沿粒子自身运动线反推与检测面的交点（hit=segEnd-curVel*(zEnd/vn)，
//           直线运动下精确=真实过面点）；规则3同改。同一光束的所有粒子（无论规则1/2/3
//           哪条抓到）映射到同一个过面点→出口是一根连续直线，散射归零。切线飞行
//           （法向速度≈0）退化为沿法线投影。
//           附带释疑（用户问"一会规则1一会规则2，我明明是World"）：规则号=哪条【检测
//           规则】抓到的粒子（1=穿越帧线段、2=框内+门后补抓、3=未配对兜底、4=反弹重建），
//           与Simulation Space无关，World粒子按进入方式不同会被不同规则抓到。
//           另有两条用户观察的定性：
//           - 速度100几乎不漏：每帧1.7米，漂移入框发生在浅深度、快进快抓；
//             速度1000每帧16.7米，漂移深、被抓前可见帧数多（但滞留=0不漏抓）。
//           - teleportTriggerOffset 是【玩家】传送触发偏移，与粒子无关（粒子检测面偏移是
//             particleTeleportPlaneOffset）；用户曾把它调到0排查，需调回0.3——
//             十五轮文档记录过0会让玩家头卡进"近门渲染退化区"。
//     二十二轮（2026-08-18，出射锚定修复：规则2/3捕获点投影回检测面 + 日志加入深/出射面距）：
//           用户实测（速度1000、187粒/秒、光束与门面有夹角）报两个现象：
//             1) "两根筷子"：出口光束与入口光束平行但整体错位，夹角越大越明显；
//             2) 入口门后面仍有可见隧穿粒子（但汇总"门后框内滞留=0"——它们最终都被传送了）。
//           破案（事件日志出射坐标自证）：规则2/3捕获"已在门后深处"的粒子时，把粒子【当前
//           深位置】直接当入口点做映射——深度d沿出口门法线被带出去：出射点=出口门中心+
//           (d+landingPush)*出口法线。速度1000+斜射时漂移入框发生在深处(d可达30~60米)，
//           出口光束被推离出口门30~60米=两根筷子；规则2与规则1的出射聚成两个分离的团。
//           修复：捕获点投影回检测面（hitPoint=segEnd-normal*zEnd；规则3同理hit=curPos+
//           forward*(offset-z)）——深度是采样伪影不是物理位置。投影后出射锚定出口门面，
//           与规则1/规则4的锚点语义完全一致（规则4一直就是投影锚定）。
//           残余现象诚实声明：投影修复的是【出射位置错位】；"门后可见几帧再被抓"仍存在——
//           粒子必须等【进入门框】那帧才被捕获（在框外飞多久可见多久），这是帧采样的
//           几何边界，采样率追不上无限快，但不会漏抓（滞留=0）。
//           日志升级：事件行新增【入深】（捕获时离入口检测面的深度，规则1≈0）与
//           【出射面距】（出射点到出口检测面的距离，应≈0.2=landingPush；远大于此=锚定异常）。
//     二十一论（2026-08-18，门洞捕获语义修正：规则2无限深度 + 诊断加框边距/已传过）：
//           用户实测速度1000仍漏（几秒一粒），嫌疑行签名：框内A=False、zA=-0.28（刚穿面
//           在框外）、配对=True——粒子在框外穿面、之后横向漂进框，但漂进框时深度已远超
//           任何有限窗（速度1000时每帧16.7米）。
//           语义修正：门洞捕获 =【门框内】且【门平面后侧】，与深度无关。深度窗是历史遗留
//           （六轮设计），而"框内"才是正确且足够的守卫——框外的墙后粒子永不误抓，
//           无限深度不会把不该传的粒子从B门墙上凭空拽出来。规则2改为：
//           深度不限（particleTeleportRetroWindow<=0，新默认-1）；>0退化为老式深度上限
//           （旧场景序列化值0.5需手动改回0/-1，否则会退回有限窗行为）。
//           数学完备性（对"进入门洞体积"的粒子，直线飞行任意速度零漏）：
//             规则1=穿越帧线段插值（穿越点框内=必抓，速度无关）；
//             规则2=框内+门后（无限深度，覆盖一切"框外穿面后漂进框"）；
//             规则3=未配对+框内+门后（无限深度兜底）；
//             规则4v2=反弹点重建（带碰撞粒子）。
//           四规则合起来覆盖全部"粒子进入门洞"的方式；不进门洞的粒子物理上就不该传。
//           诊断升级：漏检嫌疑行新增【框边距】（负=框外这么远；<0.2擦边/>1射偏或旧瞄准历史）
//           与【已传过】（该种子是否已被传送过——已传过的粒子是从出口飞过墙区，不是入口漏捕）。
//           配对池新增 particlePrevTeleported 标记（种子换人时自然失效）。
//     二十轮（2026-08-18，泛用性修正：后侧追补窗自适应 + 帧内去重）：
//           1) 用户质疑"窗要手动按速度调=破坏预制件泛用性"——采纳。改为自适应：
//              生效窗=max(基础值particleTeleportRetroWindow(默认0.5), 本帧活粒子最大速度×dt×2)。
//              原理：直线粒子穿面后首个后侧采样点深度必然<本帧位移(速度×dt)，窗=2倍位移保证
//              至少一帧采样点落在窗内——任意速度最多穿模一帧就被规则2看到，零配置。
//              低速由基础窗兜底；框内是硬约束（框外墙后粒子永不误抓）。
//              诚实边界：Local系统buffer速度为局部坐标，Transform无缩放时模长=世界速度；
//              有非1缩放时可能低估→基础窗兜底（罕见，可手动加大基础窗）。
//           2) 帧内去重：同一系统可能同时出现在root扫描列表与放置发现列表（定时刷新重填
//              root列表时不查放置列表），同帧处理两次→检测计数翻倍+配对记录重复写回。
//              新增 processedSystemsThisFrame[64] 每帧重置，ProcessSingleParticleSystem
//              开头查重跳过。这也解释了此前"检测11522颗次"虚高的问题。
//     十九轮（2026-08-18，"漂移入框"漏捕破案：后侧追补窗从默认关改为默认0.5）：
//           用户实测（速度10、World、无碰撞、90°门、直射B门）：60帧窗口内传送408次、
//           门后框内滞留423颗次；漏检嫌疑行签名一致——框内B=True、配对=True、
//           zB=-0.25~-0.33（已过检测面浅深度）。
//           破案：粒子擦门边飞行时在【框外】穿过检测面（规则1穿越点检查不认），穿过之后
//           才横向飘进门框后侧——飘进框时已无穿越事件，规则3要求未配对（它是配对粒子）、
//           规则4要反弹（没开碰撞），四规则全哑火 → 粒子永远卡在门后。
//           速度越快越明显的机制：速度10每帧0.167米，漂移入框发生在浅深度；速度100每帧
//           1.67米，穿越框外那帧终点已甩到深处/直接飞出墙外。
//           修正：1) particleTeleportRetroWindow 默认 0→0.5（速度<=100推荐0.3~0.5，
//              高速/大角度斜射1.0）——本窗专补"已过检测面且深度<=窗值且框内"的漂移入框；
//              2) 六轮"窗=0也一个不漏"的论断修订为"仅正对门框直射成立"；
//              3) 滞留计数改为【框内才算漏】（框外后侧=光束射偏穿墙，正确行为不再计入）；
//              4) 漏检嫌疑的框内判定同步使用十八轮尺寸外扩。
//           注意：场景里已序列化的窗值不会跟随代码默认值变化——旧场景 Inspector 若仍显示
//           0，手动改成0.3~0.5（除非确实想要零窗）。
...
//           窗口，规则1（没穿过检测面）与规则4（窗口）双失效。数值模拟实测：旧窗口对
//           反弹事件漏捕90.2%（5万随机样本，速度5~3000、恢复系数0.1~0.9）。
//           新算法（TryBounceCapturePortal）：速度反转（朝门→离门，阈值±0.1）时沿门法线
//           反解反弹时刻与反弹点 p=p0+v0*tb，tb=(z1-z0-vn1*dt)/(vn0-vn1)（分母恒负稳定）。
//           验证链：tb∈(0,dt) → 反弹点门框内且z∈[-0.4,1.2]容差带（覆盖碰撞半径/墙厚）→命中。
//           数值模拟验证：重构误差≈机器精度1e-14m，零越界零假阳性。速度无关、无需读碰撞
//           半径（该模块Udon不暴露）。锚点=反弹点投影到检测面，映射速度=反弹前速度。
//           "绝不漏粒子"理论审计（速度1000+，直线光束/长寿命用例）：
//             已达成：规则1线段插值对直线运动数学零漏（与帧率无关）；规则3出生兜底；
//                   规则4v2反弹重建；配对槽位按系统隔离（十六轮）。
//             文档化边界（不可修）：亚帧生灭（寿命<1帧的粒子引擎层面不可观测）；
//                   池溢出（>16系统或单系统>1024活粒子时反弹捕获降级，直线反推仍精确）；
//                   强受力曲线路径（1000m/s下重力每帧下坠≈0.0014m可忽略，低速才有）；
//                   未配对帧恰逢反弹（槽位洗牌/扩容/注册瞬间，概率极低，下一帧即恢复）。
//     十六轮（2026-08-18，白名单移除 + 自动空间分流 + 配对架构修复）：
//           1) 手动白名单 portalParticleSystems 字段整体移除：注册只剩自动收集
//              （root扫描 / 放置发现OverlapSphere / 收集根），黑名单=排除根数组不变。
//              代价（用户知情同意）：没有碰撞体的独立特效件仍要靠收集根/预制件挂隐形碰撞体。
//           2) Local/World/Custom 自动分流：编译探针实测 ps.main.simulationSpace 通过
//              UdonSharp 白名单（十五轮"白名单风险"证伪）。枚举数值已按 Unity 官方参考
//              源码核实（ParticleSystemEnums.cs）：Local=0、World=1、Custom=2。
//              ⚠️教训：首版实现把 Local/World 数值写反（World=0），导致 World 粒子被
//              双重变换坐标、Local 粒子按世界坐标直读，实测"传送 0 次"回归——枚举数值
//              必须以参考源码为准，不能按文档表格顺序猜。
//           3) 枪载粒子排除语义收窄：World空间枪载系统（枪口特效）维持自动排除防鬼畜；
//              Local空间枪载系统（激光，用户需求核心）放行参与传送。
//           4) 配对历史按系统槽位隔离（PARTICLE_PAIRING_SLOTS=16 × PARTICLE_PAIRING_STRIDE=1024
//              平铺一维数组）：修复旧版"所有系统共用一套配对数组、帧内互相覆盖→配对全体失效
//              →规则4反弹捕获永不触发"的架构缺陷。降级诚实声明：超过16个系统或单系统活粒子
//              超1024时，超出部分按未配对处理（反推线段兜底，恒速直线光束仍精确）。
//           5) 传送后防回弹：首版加了"免疫帧"（2帧内同种子跳全规则），被用户否决并移除。
//              理由：Portal 语义要求激光粒子可短时间内反复套娃穿门（A→B→A…），时间型免疫
//              会吞掉第三次合法进洞=制造新的穿墙。终稿为状态型守卫（与玩家侧同哲学，
//              玩家侧 teleportBlockFrames 默认0关闭、实际守卫是出口侧修正+lastBodySide）：
//              出口落点前推边距(2*offset+0.1)恒在出口门前侧 + 半转后出口速度必然背离
//              出口门平面 + 规则本身状态型（规则1要真实穿越/规则3要未配对/规则4要速度反转）。
//              若实测仍有"反复传送"，用 [粒子传送][事件] 日志定位是具体哪条规则在重抓。
//           6) 诊断升级：debugParticleTeleportLog 开启后每帧每颗粒子传送打一行
//              [粒子传送][事件] 规则N 门方向 种子 出射坐标——"反复传送"发生时可直接看出
//              是哪条规则在抓哪颗粒子，不用再猜。1粒/秒测试时日志密度可接受。
...
public static class 传送门中文面板_标签表
{
    ...
    public static readonly Dictionary<string, string> 门管理器标签 = new Dictionary<string, string>
    {
        ...
        { "particleTeleportRetroWindow", "粒子传送-后侧追补深度上限(<=0=不限)" },
