...
// 最后更新：2026-08-18（白名单移除+自动空间分流+配对槽位隔离+传送免疫帧，见十六轮）
// ================================================================================
//
// 【项目结构，一共3个脚本】
...
//              UdonSharp 白名单（十五轮"白名单风险"证伪）。每系统处理时读主模块：
//              World(0)直通；Local(1)经系统Transform局部↔世界转换、写回转回局部；
...
//           5) 传送后免疫帧（PARTICLE_TELEPORT_IMMUNE_FRAMES=2）：刚传送过的种子跳2帧全规则，
//              打断"同一颗粒子反复传送（停几下抽搐再走）"循环。代价：<100m/s时2帧≈3.3米，
//              两门超近距离贴身摆放时同颗粒子3.3米内的合法二次进洞会被漏掉——正常摆放无感。
