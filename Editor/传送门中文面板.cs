...
//
// 【粒子传送当前状态（2026-08-16）】
//   已通：无碰撞可传、白名单/收集根/root扫描/放置发现四路注册、实测位移+速度重建双轨、双向穿越。
//   破案记录：滞留样本显示粒子出生在A门平面后15米朝门飞——旧方向过滤(denom<-0)
//     把从平面后方穿越的粒子全部拒绝（而玩家/刚体传送都是双向的），表现为"径直穿过不传送"。
//     修复：双向穿越；回溯窗仅对朝门内飞(denom<0)生效，反向严格t∈[0,1]——
//     否则刚传送完飞离出口的粒子(t<0在窗内)会被误判回穿。
//   已知现象（非bug）：滞设计数含大量"出生在后侧的在途粒子"，样本探针只采平面后0.6米内。
// ================================================================================
#if UNITY_EDITOR
