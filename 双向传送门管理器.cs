...
public class 双向传送门管理器 : UdonSharpBehaviour
{
    ...
    private void ProcessSingleParticleSystem(ParticleSystem ps, Matrix4x4 worldToLocalA, Matrix4x4 localToWorldA, Matrix4x4 worldToLocalB, Matrix4x4 localToWorldB, int shapeA, int shapeB, float dt, float maxDistSqr)
    {
        ...
        // 配对槽位：系统池里找/占一个固定槽位，配对历史按槽位隔离（多系统互不污染）。
        // 放在距离闸门之后：只有真正参与处理的近门系统才占池，池满优先留给活跃系统。
        ...
        // 扩容翻倍后补读；配对槽位池不受扩容影响（STRIDE内的粒子配对记录保持连续），
        // 超出STRIDE的粒子走"未配对/首见"路径（规则3兜住后侧），零漏检。
        ...
        for (int i = 0; i < aliveCount; i++)
        {
            ...
            // 自动空间分流：Local/Custom系统的粒子坐标先转世界坐标再判定，写回时转回局部；
            // World系统零开销直通。
            ...
            int ruleHit = 0; // 诊断用：1=框内实穿越 2=兜底命中 3=首见后侧兜底 4=反弹捕获
            bool doA = false;
            bool doB = false;
            if (crossA || crossB)
            {
            ...
            // 诊断探针（十九轮细化，二十一论再加框边距+已传过）：
            // 滞留计数 = 已在检测面后侧【且框内】未被传送（框内才叫漏；框外=粒子根本不在门口，
            // 属光束射偏/穿墙飞走，不是检测漏，不再计入）。样本采"后侧浅深度(0~0.5m)"的未传送粒子。
            // 框边距 = 粒子中心到门框外扩边缘的距离（负=在框外这么远）：<0.2=擦边，>1=射偏/旧瞄准历史。
            // 已传过 = 该种子之前已被传送过（现在是从出口飞过的粒子，不是入口漏捕）。
            if (debugParticleTeleportLog && !wasTeleported)
            {
                ...
                if ((suspectA || suspectB) && particleDebugStuckSamples < 6)
                {
                    ...
                    Debug.Log("[粒子传送][漏检嫌疑] 系统=" + ps.name + " zA=" + zA.ToString("F3") + " zB=" + zB.ToString("F3") + " 框内A=" + inRectA + " 框内B=" + inRectB + " 框边距A=" + PortalRectMargin(localCurA, shapeA, halfSize).ToString("F2") + " 框边距B=" + PortalRectMargin(localCurB, shapeB, halfSize).ToString("F2") + " 配对=" + paired + " 已传过=" + (paired && pairIdx >= 0 && particlePrevTeleported[pairIdx]) + " 速度=" + worldVel.magnitude.ToString("F1") + " 种子=" + seed.ToString());
                }
            }

            // 记录本帧实测数据（世界坐标）：刚传送的用映射后落点，下帧线段从出口侧起算。
            // 无槽位（池满/超STRIDE）的系统不记录历史，始终走"未配对"路径。
            if (pairIdx >= 0)
            {
    ...
    private bool ParticleSegmentCrossesPortal(Vector3 segStart, Vector3 segEnd, Vector3 curVel, Transform portalPlane, Matrix4x4 worldToLocal, int shapeType, float inflateHalf, out float t, out Vector3 hitPoint, out int hitKind)
    {
        ...
        // 规则2 门洞捕获（二十一轮起无限深度）：门框内 + 门平面后侧 = 进了门洞，与深度无关。
        // 深度窗是历史遗留——漂移入框（框外穿面、之后横向飘进框）在高速时发生在很深处，
