Shader "Destruction/Portal_Skybox_TrueSync_RecursiveSebOverlay"
{
 Properties
 {
 _MainTex ("View Texture", 2D) = "white" {}
 _InactiveColour ("Seb Inactive Colour", Color) = (0, 0, 0, 1)
 _DisplayMask ("Seb Display Mask", Range(0, 1)) = 1
 ...
 o.expansion = width / _OutlineWidth;
 ...
 if(_EnableOutline < 0.5 || _DisplayMask < 0.5)
 discard;
 ...
 fixed4 outlineCol = lerp(_OutlineColor, _OutlineColor2, edgeFactor);
 ...
 // SebLague displayMask：递归最深层临时关闭传送门画面，作为递归终点。
 if(_DisplayMask < 0.5)
 {
     return _InactiveColour;
 }

 // Portal UV 计算：用当前渲染相机的视线方向算屏幕空间 UV。
 ...
 fixed4 bgColor = tex2D(_BackgroundTexture, grabUV - uvOffset);
